...
func testHost(resolver *dns.Resolver, dialer *tlsconn.Dialer, host, path string, timeout time.Duration) report {
	...
	statusCode, respLen, bodyPreview, err := requestOverConn(conn, host, path, timeout)
	if err != nil {
	...
	if !ok && bodyPreview != "" {
		line += "\n" + strings.Repeat(" ", 62) + "└─ " + truncStr(bodyPreview, 300)
	}
	return report{line: line, ok: ok}
}

// requestOverConn 在已握手的 TLS 连接上发 HTTP/1.1 GET 并读响应。
func requestOverConn(conn net.Conn, host, path string, timeout time.Duration) (int, int, string, error) {
